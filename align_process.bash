...
ls  $trimmeddata | grep -v "nobar" | sed s/_R1//g | sed s/_R2//g | sed s/.fastq// | uniq  > $home/Samplelist.$plate.txt
while read name
do    
	echo "Aligning paired $name"
    $ngm -r $ref -p --qry1 $trimmeddata/${name}_R1.fastq --qry2 $trimmeddata/${name}_R2.fastq -o $bam/$name.paired.bam -t $ncores -b
    echo "Aligning unpaired $name"
    $ngm -r $ref -q $unpaired/${name}_unR1.fastq  -o $bam/$name.R1.unpaired.bam -t $ncores -b
    $ngm -r $ref -q $unpaired/${name}_unR2.fastq  -o $bam/$name.R2.unpaired.bam -t $ncores -b
